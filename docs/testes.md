...
|---|---|---|---|---|
| **Unitários** | regra de negócio e contrato de entrada. Não abrem conexão — serviço se testa com repositório falso | `tests/unit/` | 235 | não |
...
Os sete primeiros tipos — 423 dos 544 testes — rodam **sem Docker e sem banco nenhum**. Só o que
...
```bash
make test-api                                 # os 188 testes, ~20 s, sem Docker nem banco
...
```bash
make test-unit                                      # os 235 unitários, ~3 s, sem Docker
make test-api                                       # os 188 de API, ~20 s, sem Docker
