...
|---|---|---|---|---|
| **Unitários** | regra de negócio e contrato de entrada. Não abrem conexão — serviço se testa com repositório falso | `tests/unit/` | 239 | não |
...
Os oito primeiros tipos — 445 dos 566 testes — rodam **sem Docker e sem banco nenhum**. Só o que
...
```bash
make test-api                                 # os 206 testes, ~20 s, sem Docker nem banco
...
make test-api                                       # os 206 de API, ~20 s, sem Docker
