...
`404 recurring_transaction_not_found`. Excluir uma categoria que alguma recorrência usa responde
...
| Lançamento criado com `recurrence` | vale pelo mês dele; a regra começa no mês seguinte |
...
| Regra do CRUD e da rodada | `services/recurring_transaction_service.py`; a criação junto do lançamento em `services/transaction_service.py` |
...
`tests/unit/test_recurring_transaction_service.py` e `test_transaction_service.py`; o contrato de
...
pelo nome da FK e o índice parcial em `tests/integration/test_recurring_transactions.py`.
