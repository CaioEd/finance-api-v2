...
renderizador (`core/pdf.py`, 189 linhas), a regra (`services/report_service.py`) e as rotas
...
| `core/config.py:127` | `JWT_SECRET_KEY` com menos de 32 caracteres |
...
| `core/config.py:142` | `is_production` |
...
| `core/config.py:148` | `get_settings()` com `lru_cache`; a suíte constrói `Settings` explicitamente, de propósito |
